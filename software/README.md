...
| [`scripsit/`](scripsit/README.md) | **Scripsit / SCRIPSIT/SP** — my 1988 German modification of Radio Shack Scripsit 1.0: umlauts, an `@p` + two-hex-digit printer escape, and a printer translation table, all as a length-preserving static patch in the program file. Paired with `WP/CMD`, a RAM patcher that is a **port of Craig A. Lindley's** *"Inside Scripsit"* program (80 Micro, 1982) with one extension and three routines dropped. The two layers are separated hook by hook against Lindley's binary, and the article is given in DE and EN. |
