...
Created for the preservation of an original TRS-80 Model I disk collection.
...
python3 trsextract.py --version                # print version
```
...
```
python3 trsextract.py esnd-23.dmk
```

Extract everything (note: **no spaces** in the folder name, or quote it):

```
python3 trsextract.py esnd-23.dmk -o esnd-23_extract/
```

On extraction the tool prints the geometry it auto-detected, e.g.:
...
- **G-DOS extraction** is validated; exotic G-DOS variants beyond those tested
  have not been individually confirmed.
...
The implementation cross-checks against the published NEWDOS/80 and TRSDOS
